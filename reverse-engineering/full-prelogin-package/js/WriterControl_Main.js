...
import { WriterControl_API } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterControl_API.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
...
import { WriterControl_DOMPackage } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterControl_DOMPackage.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
...
import { WriterControl_EF } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterControl_EF.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
import { WriterControl_DrawFu } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterControl_DrawFu.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
import { WriterControl_ToolBar } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterControl_ToolBar.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
import { WriterContorl_FlowChart } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterContorl_FlowChart.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
...
export let WriterControl_Main = {
    ...
    StartGlobal: function () {
        ...
        DotNet.invokeMethod(window.DCWriterEntryPointAssemblyName, "SetDefaultFont", 8731, 8731, "Arial");
        DotNet.invokeMethod(window.DCWriterEntryPointAssemblyName, "SetDefaultFont", 8322, 8322, "Calibri");
        // 罗马数字
        DotNet.invokeMethod(window.DCWriterEntryPointAssemblyName, "SetDefaultFont", 8544, 8559, "Microsoft Sans Serif");
    ...
    CreateWriterControlForWASM: function (strContainerID, type) {
        ...
        if (!rootElement.id || rootElement.id.length === 0) {
            // 如果编辑器容器ID不存在，则打印警告【DUWRITER5_0-4879】
            console.warn("编辑器容器ID不存在，请确保容器元素具有唯一的ID属性。");
        }
        ...
        if (type == "TemperatureControl") {
            ...
            if (rootElement.getAttribute("dctype") == "DCTemperatureControlForWASM") {
                ...
                rootElement._ctl = DotNet.invokeMethod(window.DCWriterEntryPointAssemblyName, "CreateWriterControlForWASM",
        ...
        var resizeObserverFunc = function (entries) {
            // 非文档环境或移动端直接返回
            if (!rootElement.ownerDocument || (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))) {
                return;
            }
            // 存储自动缩放属性
            var autoZoomStr = rootElement.getAttribute('autozoom');
            // 关闭下拉/表格下拉
            var dropdown = rootElement.querySelector('#divDropdownContainer20230111');
            ...
            var dropdownTable = rootElement.querySelector('#DCTableControl20240625151400');
            if (dropdownTable && typeof dropdownTable.CloseDropdownTable === 'function') {
                dropdownTable.CloseDropdownTable();
            }
            // 隐藏光标并刷新标尺
            WriterControl_UI.HideCaret(rootElement);
            ...
            // 处理 DPR 变化下的 canvas 尺寸与缩放，该代码会关闭自动缩放功能，需要恢复
            if (rootElement.PageContainer && window.devicePixelRatio !== rootElement.___DCLastdevicePixelRatio) {
                // 获取CSS尺寸
                let allCanvasEle = rootElement.PageContainer.querySelectorAll("canvas");
                let PixelRatio = window.devicePixelRatio;
                if (allCanvasEle && allCanvasEle.length > 0) {
                    let oldZoom = rootElement.GetZoomRate();
                    if (oldZoom != 1) {
                        rootElement.SetZoomRate(1);
                    }
                    for (var i = 0; i < allCanvasEle.length; i++) {
                        const cssWidth = allCanvasEle[i].getAttribute("native-width");
                        const cssHeight = allCanvasEle[i].getAttribute("native-height");
                        //console.log(2222222, cssWidth, cssHeight)
                        // 设置实际像素尺寸（考虑设备像素比）
                        allCanvasEle[i].width = cssWidth * PixelRatio;
                        allCanvasEle[i].height = cssHeight * PixelRatio;
                        // 设置CSS显示尺寸
                        allCanvasEle[i].style.width = cssWidth + 'px';
                        //allCanvasEle[i].style.height = cssHeight + 'px';
                        const ctx = allCanvasEle[i].getContext('2d');
                        // 缩放绘图上下文
                        ctx.scale(PixelRatio, PixelRatio);
                    }
                    rootElement.___DCLastdevicePixelRatio = PixelRatio;
                    rootElement.__DCWriterReference.invokeMethod("set_WASMBaseZoomRate", PixelRatio);
                    rootElement.RefreshDocument();
                    rootElement.SetZoomRate(oldZoom);
                }
            }
            // 恢复自动缩放属性
            if (autoZoomStr) {
                rootElement.setAttribute('autozoom', autoZoomStr);
            }
            // 不可见则跳过自动缩放
            var rects = rootElement.getClientRects ? rootElement.getClientRects() : null;
            ...
            if (WriterControl_UI && typeof WriterControl_UI.AdjustCustomDocumentCommentStyle === 'function') {
        ...
        var DebounceResizeObserverFunc = DCTools20221228.DebounceFn(resizeObserverFunc, 100);
        //zhangbin 20230607 当最外层包裹的div元素大小改变的监听事件
        if (rootElement.resizeObserver) {
            rootElement.resizeObserver.disconnect();
        }
        rootElement.resizeObserver = new ResizeObserver(DebounceResizeObserverFunc);
        //确保该元素身上只有一个事件监听
...
///**
// * 设置打印用的SVG代码，用于替换单选框的勾选图片
// * @param {string} strKey 图片名称
// * @param {string} strSVGCdoe SVG代码
// */
