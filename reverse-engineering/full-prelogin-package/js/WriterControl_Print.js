...
import { DCBinaryReader } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=DCTools20221228.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
import { PageContentDrawer } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=PageContentDrawer.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
...
import { WriterControl_UI } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterControl_UI.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
...
export let WriterControl_Print = {
    ...
    */
        ...
        if (pdfTasks == null) {
            ...
            function ExecuteNextTask() {
                ...
                if (typeof (curTask) == "string" && curTask.indexOf("data:image") == 0) {
                    ...
                    drawer.EventSource = "SaveLocalPDF";
                    ...
                    if (window.__LocalFontsErrorFlag != true && window.queryLocalFonts) {
                        try {
                            ...
                            window.queryLocalFonts().then(function (list2) {
                                ...
                                if (bolMatch == false) {
                                    WriterControl_Print.DownloadSingleFontFile(rootElement, curTask, ExecuteNextTask);
                                }
                            }).catch(function (ext2) {
    ...
    /**
     * 下载单个字体文件并设置到DotNet环境中。
     * @param {HTMLElement} rootElement - 根元素对象，用于错误处理和事件通知。
     * @param {Object} fontTask - 字体任务对象，包含字体文件名、字体名称和键值。
     * @param {Function} [ExecuteNextTask] - 任务完成后的回调函数，可选。
     */
    DownloadSingleFontFile: function (rootElement, fontTask, ExecuteNextTask) {
        // 默认字体配置
        var defaultFont = {
        ...
        // 如果 fontTask 为空，创建默认配置对象
        if (fontTask == null) {
            fontTask = {
                FileName: defaultFont.FileName,
                Name: defaultFont.Name,
                KeyValue: defaultFont.KeyValue
            };
        }
        // 如果字体文件名为空，使用默认字体配置
        else if (fontTask.FileName == null || fontTask.FileName.length == 0) {
            fontTask.FileName = defaultFont.FileName;
            fontTask.Name = defaultFont.Name;
            fontTask.KeyValue = defaultFont.KeyValue;
        }
        /**
         * 处理字体下载失败的通用错误处理函数
         * @param {HTMLElement} rootElement - 根元素对象
         * @param {string} strUrl - 字体文件URL
         * @param {number} [statusCode] - HTTP状态码，可选
         * @param {Function} [ExecuteNextTask] - 任务完成后的回调函数，可选
         */
        function handleDownloadError(rootElement, strUrl, statusCode, ExecuteNextTask) {
            if (rootElement != null) {
                var errorMessage = window.__DCSR.FailToDownLoadFontFileForPDFOFD;
                if (statusCode != null) {
                ...
                if (typeof rootElement.EventOfPDFFontDownloadError == "function") {
                    rootElement.EventOfPDFFontDownloadError(errorMessage);
                }
            }
            // 如果存在下一个任务函数，则调用它。
         ...
         * 处理字体下载成功的通用处理函数
         * @param {Object} fontTask - 字体任务对象，包含字体文件名、字体名称和键值
         * @param {Blob} blob - 字体文件blob对象
         * @param {Function} [ExecuteNextTask] - 任务完成后的回调函数，可选
         */
        function handleDownloadSuccess(fontTask, blob, ExecuteNextTask) {
        ...
        // 如果设置了全局回调函数，优先使用它
        if (WriterControl_Event.HasControlEvent(rootElement, "EventDownloadSingleFontFile") == true) {
            /**
            * 自定义字体文件下载事件处理器
            * 如果设置了此函数，将覆盖默认的下载单个字体文件并设置到DotNet环境中的行为
            * @param {HTMLElement} rootElement - 根元素对象
            * @param {Object} fontTask - 字体任务对象，包含字体文件名、字体名称和键值
            * @param {Function} setFontContentCallback - 设置字体文件内容的回调函数，参数为(keyValue, name, buffer)
            */
            const args = {
                FontTask: fontTask,
                SetFontContentCallback: function (blob) {
                    handleDownloadSuccess(fontTask, blob, ExecuteNextTask);
                }
            };
            WriterControl_Event.RaiseControlEvent(rootElement, "EventDownloadSingleFontFile", args, "Object");
            return;
        }
        // 构建字体文件的URL。
        ...
        xhr.onload = function () {
            ...
            if (this.status == 200 || this.status == 304) {
                ...
                // 检查blob是否有效（用于PDF/OFD生成场景）
                if (blob == null || blob.size < 100) {
                    // 使用通用错误处理函数
                    handleDownloadError(rootElement, strUrl, null, ExecuteNextTask);
                } else {
                    // 使用通用成功处理函数
                    handleDownloadSuccess(fontTask, blob, ExecuteNextTask);
                }
                ...
                // 请求失败，使用通用错误处理函数
                handleDownloadError(rootElement, strUrl, this.status, ExecuteNextTask);
            }
        };
        // 当请求出错时，执行错误处理。
    ...
    DownLoadFontsForLocalPDF: function (rootElement, callBack, documentsXml) {
        ...
        for (var i = 0; i < FontsArray.length; i++) {
            if (i == FontsArray.length - 1) {
                WriterControl_Print.DownloadSingleFontFile(rootElement, FontsArray[i], callBack);
            } else {
                WriterControl_Print.DownloadSingleFontFile(rootElement, FontsArray[i]);
            }
        }
    },
    ...
    PrintToServer: function (containerID, strPrintServicePageUrl, options, callBack, getprinter) {
        ...
        if (strPrintServicePageUrl == null || strPrintServicePageUrl.length == 0) {
            var advmode = options && options.AdvancedMergeMode === true;
            if (advmode === true) {
                strPrintServicePageUrl = DCTools20221228.GetLocalServicePageUrl(rootElement);
            }
            else {
        ...
        //wyc20240717:20251210添加特殊5代请求标记方便CS端后台同时处理4代和5代的请求
        if (getprinter === true) {
            strPrintServicePageUrl = strPrintServicePageUrl + "?getprinter=true&gen5=true";
        } else {
            
            strPrintServicePageUrl = strPrintServicePageUrl + "?gen5=true";
        }
    ...
    InnerDrawOnePage: function (element, bolPrintPreview, oldCtl) {
        ...
        if (strSVG != null) {
            ...
            //DUWRITER5_0-4851 问题日，打印预览设置的水印和编辑模式下有明显区别此处直接使用编辑下的水印图片
            //console.log(element, element.querySelector("[iswatermark]"))
            //let watermarkEle = element.querySelector("[iswatermark]")
            //if (watermarkEle) {
            //    //获取编辑模式下的水印图片
            //    let eleIndex = Array.prototype.indexOf.call(element.parentElement.children, element)
            //    if (eleIndex >= 0 && rootElement.PageContainer.children && rootElement.PageContainer.children.length > 0) {
            //        let newwatermark = rootElement.ownerDocument.defaultView.getComputedStyle(rootElement.PageContainer.querySelectorAll("canvas")[eleIndex], null).backgroundImage
            //        if (newwatermark != "none") {
            //            newwatermark = newwatermark.substring(5, newwatermark.length - 2);
            //            //找打对应的rect
            //            let rectEle = watermarkEle.nextElementSibling;
            //            let eleWidth = element.getAttribute("native-width") || element.getAttribute("width")
            //            let eleHeight = element.getAttribute("native-height") || element.getAttribute("height")
            //            if (watermarkEle.nodeName == "text") {
            //                let newRectEle = document.createElementNS("http://www.w3.org/2000/svg", "rect")
            //                //let svgStyle = element.getBBox();
            //                newRectEle.setAttribute("x", "0");
            //                newRectEle.setAttribute("y", "0");
            //                newRectEle.setAttribute("width", eleWidth);
            //                newRectEle.setAttribute("height", eleHeight);
            //                watermarkEle.insertAdjacentElement('afterend', newRectEle);
            //                rectEle = newRectEle
            //            }
            //            rectEle.setAttribute("fill", "rgba(255,255,255,0.1)");
            //            //创建一个图片元素并插入
            //            let newImageEle = document.createElementNS("http://www.w3.org/2000/svg", "image")
            //            //let svgStyle = element.getBBox();
            //            newImageEle.setAttribute("x", "0");
            //            newImageEle.setAttribute("y", "0");
            //            newImageEle.setAttribute("width", eleWidth);
            //            newImageEle.setAttribute("height", eleHeight);
            //            newImageEle.setAttribute("href", newwatermark)
            //            rectEle.insertAdjacentElement('beforebegin', newImageEle);
            //            //删除element
            //            watermarkEle.remove();
            //        } else {
            //            watermarkEle.remove();
            //        }
            //    } 
                //<image iswatermark="true" x="2" y="167" width="788" height="788" href="" transform="rotate(45,396,561)"></image>
                //<rect x="0" y="165" width="792" height="792" transform="rotate(45,396,561)" stroke="none" fill="rgba(255,255,255,0.8627)"></rect>
                //<rect x="0" y="0" width="793.9199" height="1122.2399" fill="url(#dcw_23)" stroke="none"></rect>
    ...
    Print: function (containerID, options) {
        ...
        // 添加打印前未被渲染回去的回调函数options.BeforePrintRenderCallback，用于在打印前处理内容【DUWRITER5_0-4917】
        if (options != null && typeof (options.BeforePrintRenderCallback) === "function") {
            options.BeforePrintRenderCallback.call(rootElement, targetDocument);
        }
        ...
        var printFun = function () {
            // 修改成确保页面个数不变的情况下才调用区域选择打印功能【DUWRITER5_0-5123】
            if (rootElement.RectInfo && typeof rootElement.RectInfo.printPreviewFun === "function") {
                // 检查 allCanvas 和 datas 是否存在且为数组
                const allCanvas = rootElement.RectInfo.allCanvas;
                const isAllCanvasValid = Array.isArray(allCanvas) && allCanvas.length > 0;
                const isDatasValid = Array.isArray(datas) && datas.length > 0;
                // 确保页面数量一致且都大于0，才调用printPreviewFun
                if (isAllCanvasValid && isDatasValid) {
                    const pageCount = allCanvas.length;
                    // 验证页面数量一致
                    if (pageCount === datas.length && pageCount > 0) {
    ...
    SetBoundsSelectionViewMode: function (containerID, boolean, reverse) {
        ...
        function AdjustBoundsSelectionStyle() {
            ...
            for (var i = 0; i < NeedAdjustCanvass.length; i++) {
                ...
                // 修复区域选择打印无效问题【DUWRITER5_0-4868】
                // 确保 askPageCanvas 的 zIndex 比 pageNode 高 1 层
                // 处理 pageNodeStyles.zIndex 不存在或无效的情况，默认使用 1
                var pageZIndex = parseInt(pageNodeStyles.zIndex, 10);
                maskPageCanvas.style.zIndex = isNaN(pageZIndex) || pageZIndex < 0 ? 2 : pageZIndex + 1;
            }
            /** 获取元素所有的样式对象
            * @param {node} node 元素
    ...
    * @param {Function} onPage 页面回调函数
    * @param {Function} onCell 单元格回调函数
    */
    walkThroughPreviewDoc: function (doc, onPage, onCell) {
        ...
        // 查找页面元素
        let pageHtml = doc.querySelectorAll("body > [pageindex]");
        ...
        // 遍历所有页面
        pageHtml.forEach((pageDOM, index) => {
            const pageId = pageDOM.getAttribute("pageindex");
            if (pageId && typeof onPage === 'function') {
                onPage(pageId, pageDOM);
            }
            // 查找表格元素
            const tables = pageDOM.querySelectorAll('[dctype="XTextTableElement"]');
            let tableDOM = null;
            if (tables.length > 1) {
                // 处理表格表头重复出现问题，取最后一个
                tableDOM = tables[tables.length - 1];
            } else if (tables.length === 1) {
                tableDOM = tables[0];
            }
            if (tableDOM) {
                const trs = tableDOM.querySelectorAll("tr");
                trs.forEach((trDOM, rowIndex) => {
                    const tds = trDOM.querySelectorAll("td");
                    tds.forEach((tdDOM, colIndex) => {
                        if (typeof onCell === 'function') {
                            onCell(pageId, rowIndex, colIndex, tdDOM);
                        }
                    });
                });
            } else {
     ...
     * @param {Array} oldStatus 老状态
     * @param {Array} newStatus 新状态
     * @returns {Array|undefined} 本次需要新增的打印状态部分
     */
    comparePrintStatus: function (oldStatus, newStatus) {
        ...
        // 深拷贝防止状态污染
        const oldPages = JSON.parse(JSON.stringify(oldStatus));
        const newPages = JSON.parse(JSON.stringify(newStatus));
        const printCell = [];
        newPages.forEach((newPage, index) => {
            const oldPage = oldPages[index];
            if (oldPage) {
                // 计算新增的打印单元格
                const newPageCells = newPage.printCells || [];
                const oldPageCells = oldPage.printCells || [];
                // 使用Set提高查找效率
                const oldPageCellsSet = new Set(
                    oldPageCells.map(cell => JSON.stringify(cell))
                );
                const newCells = newPageCells.filter(cell => 
                    !oldPageCellsSet.has(JSON.stringify(cell))
                );
    ...
    /**
    * 打印单元格
    * @param {Document} doc 打印的document
    * @param {Array} hiddenCellData 需要隐藏的单元格数据
    * @param {boolean} EnableHide 是否启动隐藏
    * @returns {Array|false} CellData 打印过的单元格数据
    */
    TemplatePrintingWithCells: function (doc, hiddenCellData, EnableHide) {
        ...
        if (EnableHide === false) {
            const styleDom = doc.querySelector("style#TemplatePrintingWithCellsStyle");
            if (styleDom) {
                styleDom.innerHTML = "";
            }
            return false;
        ...
        const CellData = [];
        // 遍历文档收集打印数据
        WriterControl_Print.walkThroughPreviewDoc(doc, 
            function (pageId, pageDOM) {
                // 发现新page的回调
                const page = { pageId: pageId, printCells: [], printed: false };
                CellData.push(page);
            }, 
            function (pageId, cellRow, cellCol, tdDOM) {
                const page = CellData.find(p => p.pageId === pageId);
                if (!page) return; 
                // 处理零宽字符，包括常见的零宽空格（U+200B）、零宽非连接符（U+200C）、零宽连接符（U+200D）、U+2002等
                const txt = tdDOM.innerText.replace(/[\u2002\u200B\u200C\u200D\uFEFF]/g, "");
                // 修复存在图片套打的情况【DUWRITER5_0-4768】
                const imgNode = tdDOM.querySelector("img");
                // 判断单元格内容是否需要打印：有非零宽字符的文本，或有图片
                if ((txt && txt.trim().length > 0) || imgNode) {
                    page.printCells.push([cellRow, cellCol]);
                }
            }
        );
        const printCell = WriterControl_Print.comparePrintStatus(hiddenCellData, CellData);
        if (!printCell) {
            return CellData;
        }
        const printedPage = {}; // 需要处理的页 {0:true}
        // 处理dom结构，用来打印
        WriterControl_Print.walkThroughPreviewDoc(doc, 
            function (pageId, pageDOM) {
                // 清除之前的样式
                pageDOM.classList.remove("hiddenPage", "printedPage");
                // 获取到当前页的打印数据
                const page = printCell.find(p => p.pageId === pageId);
                if (!page) {
                    // 数据中没有当前页
                    pageDOM.classList.add("hiddenPage");
                } else if (page.printed === true) {
                    // 之前已打印，需要处理
            ...
            function (pageId, cellRow, cellCol, tdDOM) {
                // 清除之前的样式
                tdDOM.classList.remove("printCell", "hiddenCell");
                // 处理已打印页
                if (printedPage[pageId]) {
                    const page = printCell.find(p => p.pageId === pageId);
                    if (page && page.printCells.length > 0) {
                        // 使用Set提高查找效率
                        const pageCellsSet = new Set(
                            page.printCells.map(cell => JSON.stringify(cell))
                        );
                        const cellKey = JSON.stringify([cellRow, cellCol]);
                        if (pageCellsSet.has(cellKey)) {
                            // 需要打印
        ...
        // 创建或更新样式
        let styleDom = doc.querySelector("style#TemplatePrintingWithCellsStyle");
        if (!styleDom) {
        ...
        // 构建样式字符串
        const styleRules = [
            ".hiddenPage{display: none!important;}", // 处理隐藏的页
            ".printedPage [dcpart='pagecontent'] > * {visibility: hidden!important;} .printedPage #divAllContainer{visibility: visible!important;}", // 处理页眉页脚
            ".printedPage table,.printedPage tr,.printedPage td{border-color: transparent!important;}", // 处理边框
            ".printedPage td{visibility: hidden!important;}.printedPage td.printCell{visibility: visible!important;}" // 处理单元格
        ];
        styleDom.innerHTML = styleRules.join('');
        return CellData;
    }
};
