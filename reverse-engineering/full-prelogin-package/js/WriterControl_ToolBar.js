...
export let WriterControl_ToolBar = {
     ...
     * @param {object[]} btnList 按钮列表
     */
    SetCustomizeBtn: function (containerID,btnList) {
        var rootElement = DCTools20221228.GetOwnerWriterControl(containerID);
        //找到工具条元素
        let toolsbar = rootElement.querySelector(".DC-toolBar-panel-toolsbar");
        if (toolsbar&&btnList&&btnList.length>0) {
            let CustomizeGroupDOM = rootElement.ownerDocument.createElement("div");
            //添加自定义按钮组
            CustomizeGroupDOM.classList.add("toolbar-group");
            //添加自定义按钮
            for (let i = 0; i < btnList.length; i++) {
                let btn = btnList[i];
                let btnBoxDOM = rootElement.ownerDocument.createElement("div");
                btnBoxDOM.classList.add("toolbar-box");
                btnBoxDOM.title=btn.label;
                //添加图标
                if (btn.svg) {
                    let svgDOM = rootElement.ownerDocument.createElement("div");
                    svgDOM.classList.add("only-toolbar-box-svg");
                    svgDOM.innerHTML = btn.svg;
                    btnBoxDOM.insertAdjacentElement("beforeend", svgDOM);
                } else {
                    btnBoxDOM.innerText = btn.label;
                }
                //添加点击事件
                if (btn.clickBack) {
                    btnBoxDOM.onclick =btn.clickBack
                }
                CustomizeGroupDOM.insertAdjacentElement("beforeend", btnBoxDOM);
            }
            toolsbar.insertAdjacentElement("afterbegin", CustomizeGroupDOM);
        }
       
        // let btnBoxDOM = rootElement.ownerDocument.createElement("div");
        // btnBoxDOM.classList.add("toolbar-box");
        // btnBoxDOM.title='test'
        // btnBoxDOM.innerText='保存'
        // btnBoxDOM.onclick = (e) => {
        //     console.log(e);
        // }
        // CustomizeGroupDOM.insertAdjacentElement("beforeend", btnBoxDOM);
...
let buildColorPicker = function (e, commendName, rootElement) {
    showMaskBg(rootElement);
    e.stopPropagation();
    // 使用原生方法查找最近的 .toolbar-box 元素
    const targetElement = e.target;
    const closestToolbarBox = targetElement.closest(".toolbar-box");
    if (!closestToolbarBox) {
        // console.error("未找到 .toolbar-box 元素");
        return;
    }
    let colorParentDiv = rootElement.querySelector(".DC-renderColorPickerBox");
    let oldflag = true;
    ...
    colorParentDiv.style.left = closestToolbarBox.offsetLeft + "px";
    colorParentDiv.style.top = closestToolbarBox.offsetTop + closestToolbarBox.height + "px";
