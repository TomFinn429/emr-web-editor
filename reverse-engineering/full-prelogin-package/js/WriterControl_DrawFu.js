import { d3 } from "http://10.0.15.23:8084/MyWriter/MoreHandleDCWriterServicePage?wasmres=WriterControl_DrawD3.js&ver=20260327143150_409&flag=293105&wasmrootpath=http%3A%2F%2F10.0.15.23%3A8084%2FMyWriter%2FMoreHandleDCWriterServicePage";
...
export let WriterControl_DrawFu = {
    ...
    CreateDefaultData: function (rootElement, data) {
        ...
        var DefaultData = {
            ...
            NumOfPages: 0,
            Config: {
                "DateAutoFillPageProperties": false, //日期自动填充整页属性
    ...
    ChangeValue: function (rootElement, data) {
        ...
        //在此处计算差值，差值为天数如果天数不能整除calculationData.NumOfDaysInOnePage，则把差值天数补齐
        if (defaultData.DateAutoFillPageProperties) {
            var diffDays = (calculationData.HeaderLineEndDateTime - calculationData.HeaderLineStartDateTime) / 1000 / 60 / 60 / 24;
            if (diffDays % calculationData.NumOfDaysInOnePage != 0) {
                //把差值天数补齐
                var diffDays = Math.ceil(diffDays / calculationData.NumOfDaysInOnePage) * calculationData.NumOfDaysInOnePage;
                calculationData.HeaderLineEndDateTime = new Date(calculationData.HeaderLineStartDateTime.getTime() + diffDays * 1000 * 60 * 60 * 24);
            }
        }
    ...
    DrawSvg: function (rootElement) {
        ...
        //20251204 lxy 兼容讯飞需求,第某次手术当天用罗马数字表示，后续用阿拉伯数字表示
        //将whichTime初始化移到页面循环之前，使其能在所有页面之间共享，实现跨页累积手术次数
        if (!calculationData.whichTime) {
            calculationData.whichTime = 0;
        }
        ...
        for (var i = 0; i < calculationData.TotalPageNumber; i++) {
            ...
            WriterControl_DrawFu.DrawYAxisInfos(rootElement, svg, calculationData, defaultData, svgEle, i);
    ...
    DrawLabel: function (svg, calculationData, defaultData, svgEle, i, parameters) {
        ...
        if (defaultData.Labels && defaultData.Labels.length > 0) {
            ...
            for (var j = 0; j < defaultData.Labels.length; j++) {
                ...
                if ((thisLabel.Text == "疼痛强度" || thisLabel.Text == "疼痛评分" || thisLabel.Text == "疼痛程度") && ISDefaultTemperatureMode) {
                ...
                if (thisLabel.ImageDataBase64String && thisLabel.ImageDataBase64String.length > 0) {
                    ...
                    var textEle = thisLabelG.append('text')
                        .attr('style', `font-family:${thisLabel.TextFontName || '宋体,国标宋体'} ;font-size:${thisLabel.TextFontSize || defaultData.FontSize}pt;font-weight:${thisLabel.TextFontBold ? 900 : 100};text-decoration: ${thisLabel.TextFontUnderline ? 'underline' : 'none'}; font-style:${thisLabel.TextFontItalic ? 'italic' : 'none'};`)
                        .attr('dctype', 'dcLables')
                    ...
                    if (thisLabel.LineAlignment == "Center") {
                        if (thisLabel.Text == "疼痛强度" || thisLabel.Text == "疼痛评分" || thisLabel.Text == "疼痛程度") {
    ...
    DrawHeadLable: function (svg, calculationData, defaultData, svgEle, i, parameters) {
        ...
        //判断是否只展示转科后最后一个值
        var HeaderLabelChangeShowLastOne = defaultData.HeaderLabelChangeShowLastOne ? defaultData.HeaderLabelChangeShowLastOne : false;
                            ...
                            if (defaultData.HeaderLabelChangeDate && defaultData.HeaderLabelChangeDate[thisHeaderLables[k].UID]) {
                                ...
                                for (var z = 0; z < ValueChangeData.length; z++) {
                                    ...
                                    if (changeTime >= thisBand[0] && changeTime <= thisBand[thisBand.length - 1]) {
                                        //判断是否只展示转科后最后一个值
                                        if (HeaderLabelChangeShowLastOne) {
    ...
    DrawTableContainer: function (svg, calculationData, defaultData, svgEle, i) {
        ...
        if (calculationData.TemperatureMode !== "Fahrenheit") {
            ...
            //[DUWRITER5_0-4889] 20251028 新增，在 TemperatureMode=="Fahrenheit"时是否展示左侧标题边框,默认不展示false
            if (defaultData.FahrenheitYAxisInfosBorderVisible === true) {
                dcTableContainerGRect
                    .attr('stroke', "black")
            ...
            if (d.ValueType == "SerialDate" || d.ValueType == "GlobalDayIndex" || d.ValueType == "NewSerialDate") {
                ...
                if (textList.length > 0) {
                    ...
                    if (hasValue && hasValue[i]) {
                        ...
                        //增加一个属性upAndDownTextTypeDisplay设置呼吸第一个值在下方展示
                        var upAndDownTextTypeDisplay = d.UpAndDownTextTypeDisplay ? d.UpAndDownTextTypeDisplay : "FirstValueIsAbove";
                        var upAndDown = upAndDownTextTypeDisplay == "FirstValueIsBelow" ? true : false;
                                ...
                                for (var j = 0; j < thisRectValue.text.length; j++) {
                                    ...
                                    if (d.LayoutType == "Slant") {
                                        thisTextEle
                                            ...
                                            .attr('style', `font-family:${(defaultData.FontName || '宋体,国标宋体')};font-size:${thisRectValue.data[j].TextFontSize || d.TextFontSize || defaultData.FontSize}px;`);
                                        //计算0 点
                                        ...
                                        dcHeadLineTextG.append('text')
                                            .attr('style', `font-size:${thisRectValue.data[j].TextFontSize || d.TextFontSize || defaultData.FontSize}px;font-family:${d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};text-anchor:end;`)
                                            .attr('xml:space', 'preserve')
                                            .attr('fill', d.TextColorValue ? d.TextColorValue : "black")
                                        ...
                                        thisTextEle
                                            ...
                                            .attr('style', `font-family:${(defaultData.FontName || '宋体,国标宋体')};font-size:${thisRectValue.data[j].TextFontSize || d.TextFontSize || defaultData.FontSize}px;text-anchor:middle;`);
                                        ...
                                        dcHeadLineTextG.append('text')
                                            .attr('style', `font-size:${thisRectValue.data[j].TextFontSize || d.TextFontSize || defaultData.FontSize}px;font-family:${d.TextFontName ? d.TextFontName : (defaultData.FontName || '宋体,国标宋体')};text-anchor:middle;`)
                                            .attr('xml:space', 'preserve')
                                            .attr('fill', d.TextColorValue ? d.TextColorValue : "black")
                                            .attr('x', j * stepWidth + calculationData.FirstTextWidth + thisRectValue.startIndex + thisRectValue.basisWidth / 2)
                                            .attr('y', calculationData.LableHeight)
                ...
                if (d.StartDateKeyword) {
                    //20251204 lxy 兼容讯飞需求,第某次手术当天用罗马数字表示，后续用阿拉伯数字表示
                    //使用calculationData中的whichTime，使其能在所有页面之间共享
                    var whichTime = calculationData.whichTime || 0;
                    var romanText = ["I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X"];
                            ...
                            .text((j) => {
                                ...
                                for (var k = 0; k < haskeyWordTime.length; k++) {
                                    ...
                                    if (diffNum > -1) {
                                        ...
                                        //判断是否存在结束时间
                                        if (hasEndWordTime) {
                                            //enddiffNumn 的值代表当前日期与结束时间的天数差
                                            var enddiffNumn = (thisBand[j] - hasEndWordTime) / 1000 / 60 / 60 / 24;
                                            //判断是否计算出院日期
                                            if (d.CalculateEndDateKeyword === true) {
                                                //计算出院日期
                                                if (enddiffNumn > 0) {
                                                    diffNumArr = [];
                                                    break;
                                ...
                                //20251204 lxy 兼容讯飞需求,第某次手术当天用罗马数字表示，后续用阿拉伯数字表示
                                var DateIsRepresentedInRoman = rootElement.getAttribute("DateIsRepresentedInRoman") || false;
                                if (DateIsRepresentedInRoman) {


                                    //处理产后天数，只记录14天
                                    diffNumArr = diffNumArr.map((item, index) => {
                                        if (item === 0) {
                                            whichTime++;
                                            calculationData.whichTime = whichTime; //更新全局计数器
                                            if (whichTime === 1) {
                                                item = "";
                                            } else {
                                                // item = romanText[whichTime - 1];
                                                item = "II";
                                    ...
                                    //手术天数用罗马数字表示
                                    diffNumArr = diffNumArr.reverse();
                                    if (diffNumArr && diffNumArr.length > 0) {
                                        if (diffNumArr && diffNumArr[diffNumArr.length - 1] > 14) {
                                            diffNumArr = [];
                                        }
                                    }
                                    //删除大于14的值
                                    diffNumArr = diffNumArr.filter((item, index) => {
                                    ...
                                    //保留最后两个值
                                    if (diffNumArr && diffNumArr.length > 2) {
                                        diffNumArr = diffNumArr.slice(-2);
                                    }
                                    console.log(diffNumArr);
                                    //判断diffNumArr的长度是否为1，且diffNumArr[0]是否为0
                                    if (diffNumArr.length === 1) {
                                        var firstValue = diffNumArr[0];
                                        if (firstValue && typeof firstValue === "string" && firstValue === "II") {
                                            diffNumArr = [];
                                        }
                                    }
                                }
    ...
    //绘制Y轴
    DrawYAxisInfos: function (rootElement, svg, calculationData, defaultData, svgEle, i) {
        //计算拿到Y轴总高带
        ...
        calculationData.YAxisInfosData.map((yData, yIndex) => {
            ...
            var dcYAxisInfos_Text = thisYAxisInfos.append('g')
                ...
                .html((d, i) => {
                    ...
                    if (i === 0) {
                        ...
                        if (yData[1].Title) {
                            //绘制Title和BottomTitle
                            var titleGroup = thisYAxisInfos.append('g')
                                ...
                                .text(yData[1].Title);
                            //在指定宽度后，是否需要换行
                            if (thisNameYAxis.SpecifyTitleWidth && parseFloat(thisNameYAxis.SpecifyTitleWidth) > 0) {
                                //计算指定宽度
                                var currentSpecifyTitleWidth = parseFloat((thisNameYAxis.SpecifyTitleWidth / 300 * 96.00001209449).toFixed(2));
                                //计算文本宽度
                                var currentTitleWidth = titleGroup.node().getBBox().width;
                                var textTitle = yData[1].Title;
                                if (currentTitleWidth > currentSpecifyTitleWidth) {
                                    // console.log('需要换行', textTitle, currentTitleWidth, currentSpecifyTitleWidth);

                                    // 克隆一个textTitle标签用于计算宽度
                                    var cloneTitleGroup = titleGroup.clone(true);
                                    cloneTitleGroup.attr('visibility', 'hidden');
                                    titleGroup.node().parentNode.appendChild(cloneTitleGroup.node());

                                    // 获取父元素textTitle的x属性值
                                    var parentX = titleGroup.attr('x') || 0;
                                    ...
                                    //开始换行
                                    var lastIndex = 0;
                                    for (var textIndex = 0; textIndex < textTitle.length; textIndex++) {
                                        var thisText = textTitle.substring(lastIndex, textIndex + 1);
                                        // 使用克隆的标签计算宽度
                                        cloneTitleGroup.text(thisText);
                                        var thisTextWidth = cloneTitleGroup.node().getBBox().width;

                                        if (thisTextWidth >= currentSpecifyTitleWidth) {
                                            // 需要换行，添加tspan标签
                                            var lineText = textTitle.substring(lastIndex, textIndex);
                                            if (lineText.length > 0) {
                                                var tspan = titleGroup.append('tspan').text(lineText);
                                                if (titleGroup.selectAll('tspan').size() > 1) {
                                                    tspan.attr('x', parentX).attr('dy', '1.1em');
                                                }
                                                lastIndex = textIndex;
                                                // console.log('换行添加tspan', lineText, thisTextWidth, currentSpecifyTitleWidth);
                                            }
                                        }
                                    }

                                    // 处理最后一段文本
                                    if (lastIndex < textTitle.length) {
                                        var lastLineText = textTitle.substring(lastIndex);
                                        if (lastLineText.length > 0) {
                                            var tspan = titleGroup.append('tspan').text(lastLineText);
                                            if (titleGroup.selectAll('tspan').size() > 1) {
                                                tspan.attr('x', parentX).attr('dy', '1.1em');
                                            }
                                            // console.log('最后一行', lastLineText);
                                        }
                                    }

                                    // 移除克隆的标签
                                    cloneTitleGroup.remove();
                                }
                    ...
                    if (i == yData[0].length - 1) {
                        if (yData[1].BottomTitle) {
                            ...
                            thisYAxisInfos.append('g')
                                ...
                                .attr("y", calculationData.YAxisInfosTotalHeight - thisNameYAxis.YBottomHeight - 2)
            ...
            //[DUWRITER5_0-4889] 20251028 新增，在 TemperatureMode=="Fahrenheit"时是否展示左侧标题边框,默认不展示false
            if (defaultData.FahrenheitYAxisInfosBorderVisible === true && ISDefaultTemperatureMode === false) {
                ISDefaultTemperatureMode = true;
            }
            //是否需要绘制线段
            if (yData[1].MergeIntoLeft) {
        ...
        //[DUWRITER5_0-4766] 20251010 lxy 增加自动隐藏被标题遮盖的图标和文本的功能
        //防止影响其他客户，新增一个IsHideElementsBbscuredByYaxisTitles属性，判断是否开启该功能
        var IsHideElementsBbscuredByYaxisTitles = rootElement.getAttribute('IsHideElementsBbscuredByYaxisTitles');
        IsHideElementsBbscuredByYaxisTitles = IsHideElementsBbscuredByYaxisTitles === 'true' || IsHideElementsBbscuredByYaxisTitles === true;
        if (!IsHideElementsBbscuredByYaxisTitles) {
            return;
        }
        // 循环一遍Y轴，看标题是否遮盖了图标和文本，如果遮盖了，则删除被遮盖的元素
        var allYAxisInfosG = dcYAxisInfosG.node().childNodes;
        //判断两个边界框是否重叠的函数
        var isOverlap = function (bbox1, bbox2) {
            // 计算两个矩形的边界
            var rect1 = {
                left: bbox1.x,
        ...
        //循环allYAxisInfosG数组，获取每一个元素下的子元素
        for (var i = 0; i < allYAxisInfosG.length; i++) {
            var yAxisItem = allYAxisInfosG[i];

            if (yAxisItem.childNodes && yAxisItem.childNodes.length > 0) {
                // 第一步：收集所有子元素信息并找到基准元素
                var childElements = [];
                var titleElement = null;
                var titleBBox = null;

                for (var j = 0; j < yAxisItem.childNodes.length; j++) {
                    var childElement = yAxisItem.childNodes[j];
                    ...
                    try {
                        // 获取元素的边界框信息
                        var bbox = childElement.getBBox();
                        var dctype = childElement.getAttribute('dctype') || '无';
                        // 保存子元素信息
                        childElements.push({
                        ...
                        // 找到基准元素（dctype为dcYAxisInfos_Title）
                        if (dctype === 'dcYAxisInfos_Title') {
                            titleElement = childElement;
                            titleBBox = bbox;
                        }
                    } catch (e) {
                        console.log('  子元素 ' + j + ' - 无法获取边界框信息: ' + e.message);
                    }
                }

                // 第二步：如果找到了基准元素，检查其兄弟元素是否与之重叠
                if (titleElement && titleBBox) {
                    for (var k = 0; k < childElements.length; k++) {
                        var sibling = childElements[k];
                        // 跳过基准元素本身
                        if (sibling.element === titleElement) {
                            continue;
                        }
                        // 特殊处理：如果是 dcYAxisInfos_Text，检查其子元素是否重叠
                        if (sibling.dctype === 'dcYAxisInfos_Text') {
                            if (sibling.element.childNodes && sibling.element.childNodes.length > 0) {
                                for (var m = 0; m < sibling.element.childNodes.length; m++) {
                                    var textChild = sibling.element.childNodes[m];
                                    // 跳过文本节点和注释节点
                                    if (textChild.nodeType !== 1) {
                                        continue;
                                    }
                                    try {
                                        var textChildBBox = textChild.getBBox();
                                        // 判断子元素是否与标题重叠
                                        if (isOverlap(titleBBox, textChildBBox)) {
                                            // 隐藏子元素（不删除）
                                            textChild.style.opacity = '0';
                                        }
                                    } catch (e) {
                                        console.log('    dcYAxisInfos_Text 的子元素 ' + m + ' - 无法获取边界框信息: ' + e.message);
                                    }
                                }
                            }
                        } else {
                            // 其他元素：判断整个元素是否重叠
                            if (isOverlap(titleBBox, sibling.bbox)) {
                                // 隐藏元素（不删除）
    ...
    /**
     * 处理 HollowCovertTargetName 相关的重合点逻辑
     * 当数据点与目标点重合时，将当前点显示为空心圆（覆盖在目标点上）
     * @param {Object} dcValuePathG - SVG数据路径组元素
     * @param {Array} value - 数据值数组
     * @param {Object} calculationData - 计算数据对象
     */
    ProcessHollowCovertTargetOverlap: function (dcValuePathG, value, calculationData) {
        if (!dcValuePathG || !value || !value.length) {
            return;
        }

        // 遍历所有数据点，处理重合点逻辑
        for (var j = 0; j < value.length; j++) {
            var minMax = calculationData.YAxisInfosData["dc_" + value[j].Name];
            if (!minMax || !minMax.HollowCovertTargetName) {
                continue;
            }

            // 收集目标点的坐标和title（基于坐标去重，避免同一坐标的点被多次添加）
            var allPointIndex = [];
            var coordinateSet = {}; // 用于坐标去重的对象，key格式为 "x,y"
            ...
            if (hollowCovertTargetName.length >= 1) {
                for (var l = 0; l < hollowCovertTargetName.length; l++) {
                    ...
                    // 查找所有有dcSymbolX属性的circle元素（或其他元素）
                    var allPointDOM = targetNameDOM ? targetNameDOM.querySelectorAll("circle[dcSymbolX], line[dcSymbolX], [dcSymbolX]") : null;
                    if (allPointDOM && allPointDOM.length > 0) {
                        for (var d = 0; d < allPointDOM.length; d++) {
                            var pointElement = allPointDOM[d];
                            var pointX = pointElement.getAttribute('dcSymbolX');
                            var pointY = pointElement.getAttribute('dcSymbolY');
                            if (pointX && pointY) {
                                // 格式化坐标用于去重（与后续比较逻辑一致）
                                var normalizedX = parseFloat(pointX).toFixed(2);
                                var normalizedY = parseFloat(pointY).toFixed(2);
                                normalizedX = Math.round(parseFloat(normalizedX) * 10) / 10;
                                normalizedY = Math.round(parseFloat(normalizedY) * 10) / 10;
                                var coordKey = normalizedX + ',' + normalizedY;

                                // 使用坐标作为唯一标识进行去重
                                if (!coordinateSet[coordKey]) {
                                    var pointTitle = pointElement.getAttribute('title') || '';
                                    allPointIndex.push({
                                        x: pointX,
                                        y: pointY,
                                        title: pointTitle,
            ...
            if (allPointIndex.length === 0) {
                continue;
            }

            // 获取当前数据点的所有图标元素（所有有dcSymbolX属性的元素）
            var currentValuePathDOM = dcValuePathG.node().querySelector(`[dctype=dcValuePath_${value[j].Name}]`);
            if (!currentValuePathDOM) {
                continue;
            }

            // 查找所有有dcSymbolX属性的元素（circle、line等）
            var allCurrentElements = currentValuePathDOM.querySelectorAll("circle[dcSymbolX], line[dcSymbolX], [dcSymbolX]");
            if (!allCurrentElements || allCurrentElements.length === 0) {
                continue;
            }
            // 检查每个当前点是否与目标点重合
            for (var d = 0; d < allCurrentElements.length; d++) {
                var currentElement = allCurrentElements[d];
                var currentX = currentElement.getAttribute('dcSymbolX');
                var currentY = currentElement.getAttribute('dcSymbolY');
                ...
                currentY = parseFloat(currentY).toFixed(2);
                currentX = Math.round(parseFloat(currentX) * 10) / 10;
                currentY = Math.round(parseFloat(currentY) * 10) / 10;

                // 检查是否与目标点重合，并收集所有重合点的title（去重）
                var isOverlap = false;
                var overlapTitles = [];
                var titleSet = {}; // 用于去重的对象

                // 添加当前点的title
                var currentTitle = currentElement.getAttribute('title') || '';
                if (currentTitle && currentTitle.trim()) {
                    var trimmedTitle = currentTitle.trim();
                    if (!titleSet[trimmedTitle]) {
                        overlapTitles.push(trimmedTitle);
                        titleSet[trimmedTitle] = true;
                    }
                }

                for (var p = 0; p < allPointIndex.length; p++) {
                    var targetPoint = allPointIndex[p];
                    var targetX = parseFloat(targetPoint.x).toFixed(2);
                    var targetY = parseFloat(targetPoint.y).toFixed(2);
                    targetX = Math.round(parseFloat(targetX) * 10) / 10;
                    targetY = Math.round(parseFloat(targetY) * 10) / 10;

                    if (currentX == targetX && currentY == targetY) {
                        isOverlap = true;
                        // 收集目标点的title（去重）
                        if (targetPoint.title && targetPoint.title.trim()) {
                            var trimmedTargetTitle = targetPoint.title.trim();
                            if (!titleSet[trimmedTargetTitle]) {
                                overlapTitles.push(trimmedTargetTitle);
                                titleSet[trimmedTargetTitle] = true;
                            }
                        }
                    }
                }

                // 如果重合，修改点的样式为空心圆，并重置title为所有重合点的title集合
                if (isOverlap) {
                    // 判断元素类型：circle元素可以设置为空心圆，其他元素（如line）只更新title
                    var isCircle = currentElement.tagName.toLowerCase() === 'circle';
                    if (isCircle) {
                        // 对于circle元素，设置填充为透明，半径放大1.5倍
                        var originalR = parseFloat(currentElement.getAttribute('r')) || 0;
                        if (originalR > 0) {
                            currentElement.setAttribute('fill', 'transparent');
                            currentElement.setAttribute('r', originalR * 1.5);
                            // 确保有描边，以便显示空心效果
                            var currentStroke = currentElement.getAttribute('stroke');
                            if (!currentStroke || currentStroke === 'none') {
                                // 获取父元素的stroke颜色，如果没有则使用默认颜色
                                var parentG = currentElement.closest('g');
                                var strokeColor = parentG ? parentG.getAttribute('stroke') : null;
                                currentElement.setAttribute('stroke', strokeColor || minMax.SymbolColorValue || '#ff0000');
                                currentElement.setAttribute('stroke-width', '1');
                            }
                        }
                    }
                    // 重置title为所有重合点的title集合（所有元素类型都需要）
                    if (overlapTitles.length > 0) {
                        currentElement.setAttribute('title', overlapTitles.join('\n'));
                    }
                }
            }
        }
    ...
    DrawValuePath: function (svg, calculationData, defaultData, svgEle, i, value) {
                    ...
                    if (value[j].Datas) {
                        ...
                        thisSvgDataArr.forEach((thisSvgData) => {
                            ...
                            if (minMax.Style == "Text") {
                                thisValuePath.append("g")
                                    ...
                                    .html((d, i) => {
                                        ...
                                        if (newResultHtml && newResultHtml.length) {
                                            ...
                                            var charArr = '';
                                            // Chrome 92 和 Chrome 95 特例：不添加 writing-mode: lr;
                                            var isChrome92 = /Chrome\/(92|95)\./.test(navigator.userAgent) && !/Edg\//.test(navigator.userAgent) && !/OPR\//.test(navigator.userAgent);
                                            for (var k = 0; k < newResultHtml.length; k++) {
                                                ...
                                                var style = char.match(/[a-zA-Z0-9]/g)
                                                    ? `font-size:${minSize - 1}pt;`
                                                    : `font-size:${minSize - 1}px;${isChrome92 ? '' : 'writing-mode: lr;'}`;
                                                ...
                                                // ←-↓
                                                // →-↑
                                                // 将特殊符号转换展示，防止在92版本浏览器下出现旋转问题，这里只是展示效果,不会影响到实际的数值
                                                if (isChrome92) {
                                                    if (char == '↑') {
                                                        char = '←';
                                                    } else if (char == "↓") {
                                                ...
                                                var thisY = y;
                                                //修复文本压线问题：使用HorizontStepHeight对齐，而不是minSize
                                                var num = Math.round(y / calculationData.HorizontStepHeight);//四舍五入取整数，保证文字在格子内,防止压线
                                                thisY = num * calculationData.HorizontStepHeight;
                                                //判断是否需要间距
                                                if (calculationData.HorizontStepHeight > minSize) {
                                                    thisY += (calculationData.HorizontStepHeight - minSize) / 2;
                                                }
                                                thisY += (calculationData.HorizontStepHeight * k);
                                ...
                                //[DUWRITER5_0-4771]   20250917 lxy 修改体温单文本值在92浏览器下偏移的问题                          
                                ...
                                if (allGroupText && allGroupText.length > 0) {
                                    ...
                                    //第一层所有的循环
                                    for (var z = 0; z < allGroupText.length; z++) {
                                        var groupTextItme = allGroupText[z];
                                        //当前text的X坐标
                                        var groupTextX = parseFloat(groupTextItme.getAttribute('x'));
                                        //当前text的Y坐标
                                        var groupTextY = parseFloat(groupTextItme.getAttribute('y'));
                                        //第二层循环，处理相同X的元素
                                        for (var m = 0; m < allGroupText.length; m++) {
                                            var currentGroupTextItme = allGroupText[m];
                                            var currentGroupTextX = parseFloat(currentGroupTextItme.getAttribute('x'));
                                            var currentGroupTextY = parseFloat(currentGroupTextItme.getAttribute('y'));

                                            //在同一个x区域内
                                            if (currentGroupTextX === groupTextX) {
                                                if (currentGroupTextItme !== groupTextItme) {

                                                    //判断是否重叠
                                                    var currentGroupTextItmeBox = currentGroupTextItme.getBBox();
                                                    var groupTextItmeBox = groupTextItme.getBBox();
                                                    if (groupTextY + groupTextItmeBox.height > currentGroupTextY && groupTextY < currentGroupTextY + currentGroupTextItmeBox.height) {
                                                        // 仅当当前元素位于下面（y 更大或等于）时再进行移动与追加分割符，避免对最顶部元素处理
                                                        if (currentGroupTextY >= groupTextY) {
                                                            // 覆盖的 text 需要增加一个拼接符来区分（仅在尚未添加时）
                                                            var hasGapLine = currentGroupTextItme.querySelector(".dc_textGapLine");
                                                            if (!hasGapLine) {
                                                                var isChrome92 = /Chrome\/(92|95)\./.test(navigator.userAgent) && !/Edg\//.test(navigator.userAgent) && !/OPR\//.test(navigator.userAgent);
                                                                var gapChar = isChrome92 ? '一' : '—';
                                                                var gapStyle = isChrome92 ? 'font-size: 12px;' : 'font-size: 12px;writing-mode: lr;';
                                                                currentGroupTextItme.insertAdjacentHTML("afterbegin", `<tspan class="dc_textGapLine" style="${gapStyle}"  fill="#000">${gapChar}</tspan`);
                                                            }


                                                            // 把覆盖的向后拼接
                                                            var newY = groupTextY + groupTextItmeBox.height; // 拼接开始坐标
                                                            // newY 处理为能被 calculationData.HorizontStepHeight 整除的高度(防止文字压线)
                                                            var newYNumber = Math.round(newY / calculationData.HorizontStepHeight);
                                                            newY = newYNumber * calculationData.HorizontStepHeight;

                                                            currentGroupTextItme.setAttribute('y', newY);
                                                            var allCurrentGroupTextItmeTspan = currentGroupTextItme.querySelectorAll('tspan');
                                                            for (var n = 0; n < allCurrentGroupTextItmeTspan.length; n++) {
                                                                var currentTspan = allCurrentGroupTextItmeTspan[n];
                                                                var currentTspanY = newY + (n * calculationData.HorizontStepHeight);
                                                                currentTspan.setAttribute('y', currentTspanY);
                                                            }
                                                        }
                                                    }
                                                }
        ...
        // 处理 HollowCovertTargetName 相关的重合点逻辑
        WriterControl_DrawFu.ProcessHollowCovertTargetOverlap(dcValuePathG, value, calculationData);
        ...
        if (calculationData.ShadowNameArr.List) {
            for (var z in calculationData.ShadowNameArr.List) {
                ...
                // [DUWRITER5_0 - 4781] 20250919 lxy 修复阴影区域绘制错误,在这里重新获取一遍Y轴信息
                var thisValue = calculationData.ShadowNameArr.List[z];
                //获取一遍Y轴信息
                var minMax = calculationData.YAxisInfosData[`dc_${z}`];
        ...
        return {
            ...
            P: (({
                ...
                content
                    ...
                    .attr('font-weight', 500) // 设置字体粗细为100，让字体更细
                    .attr('fill', fill)
                    .attr('stroke', 'none') // 移除描边，避免字体看起来过粗
                    .attr('stroke-width', 0) // 设置描边宽度为0
                    .attr('title', (i) => {
                        if (typeof title === 'function') {
                            return title(i);
                        }
    ...
    DrawBlockElements: function (svg, calculationData, defaultData, svgEle, i) {
        ...
        if (calculationData.TemperatureMode && calculationData.TemperatureMode === "Fahrenheit") {
            ...
            if (!defaultData.FahrenheitYAxisInfosBorderVisible) {
            ...
            //[DUWRITER5_0-4889] 20251029 在右侧占位区域画一个矩形，用于绘制华氏度
            if (defaultData.FahrenheitRightYAxisInfoTargetName && defaultData.FahrenheitRightYAxisInfoTargetName.trim() !== '') {
                var fahrenheitRightStartX = 0, fahrenheitRightEndX = blockWidth,
                    fahrenheitRightStartY = calculationData.HeaderLinesTotalHeight,
                    fahrenheitRightEndY = footerLineY - calculationData.HeaderLinesTotalHeight;

                //在右侧占位区域画一个矩形，宽高和右侧占位元素一样
                blockG.append('rect')
                    .attr('x', fahrenheitRightStartX).attr('y', fahrenheitRightStartY)
                    .attr('width', fahrenheitRightEndX).attr('height', fahrenheitRightEndY)
                    .attr('fill', 'transparent').attr('stroke', 'transparent');

                var targetYAxisData = calculationData.YAxisInfosData["dc_" + defaultData.FahrenheitRightYAxisInfoTargetName];
                if (targetYAxisData && targetYAxisData.MinValue && targetYAxisData.MaxValue) {
                    var maxValueVisible = targetYAxisData.MaxValueVisible === true;
                    var rectHeight = fahrenheitRightEndY, rectY = fahrenheitRightStartY;
                    var maxC = parseFloat(targetYAxisData.MaxValue), minC = parseFloat(targetYAxisData.MinValue);
                    var YSplitNum = targetYAxisData.YSplitNum || 1;

                    //隐藏最大值时，调整绘制区域
                    if (!maxValueVisible && YSplitNum > 1) {
                        var stepNum = (maxC - minC) / YSplitNum;
                        var stepHeight = rectHeight / YSplitNum;
                        rectHeight -= stepHeight;
                        rectY += stepHeight;
                    ...
                    //获取字体和颜色配置
                    var titleColorValue = targetYAxisData.TitleColorValue || '#000000';
                    var fontSize = (targetYAxisData.FontSize && parseFloat(targetYAxisData.FontSize) > 0) ? parseFloat(targetYAxisData.FontSize) : (defaultData.FontSize || 9);
                    var fontBold = targetYAxisData.FontBold === true || targetYAxisData.FontBold === 'true';
                    var fontStyle = `font-weight:${fontBold ? 900 : 100};font-family:${defaultData.FontName || '宋体,国标宋体'};font-size:${fontSize}pt;fill:${titleColorValue};text-anchor: start`;

                    //华氏度标题
                    blockG.append('text')
                        .attr('x', fahrenheitRightStartX + 5)
                        .attr('y', fahrenheitRightStartY)
                        .attr('style', fontStyle)
                        .text('华氏');

                    //创建华氏度标尺容器
                    var fahrenheitG = blockG.append('g')
                        .attr("dctype", "dcYAxiosInfos_FahrenheitRight")
                        .attr('transform', `translate(${fahrenheitRightStartX},${rectY})`);

                    //垂直边界线
                    var minFStartY = (Math.ceil(minF) - minF) * StepFHeight;
                    fahrenheitG.append('line')
                        .attr('x1', 0)
                        .attr('y1', 0)
                        .attr('x2', 0)
                        .attr('y2', rectHeight - minFStartY)
                    ...
                    //绘制刻度线和文本
                    var tickLineLength = 10, minorTickLineLength = 6, textOffset = 4;
                    for (var f = Math.ceil(minF); f <= maxF; f += 1) {
                        var currentY = rectHeight - ((f - minF) * StepFHeight);
                        ...
                        //文本
                        var textEl = fahrenheitG.append('text')
                            .attr('x', tickLineLength + textOffset).attr('y', currentY)
                            .attr('style', fontStyle).text(f + '°');
                        textEl.attr('y', parseFloat(textEl.attr('y')) + (textEl.node().getBBox().height / 2));

                        //小刻度线
                        if (f < maxF - 1) {
                            for (var l = 1; l < 5; l++) {
                                var minorY = currentY - (StepFHeight / 5 * l);
                                fahrenheitG.append('line')
                                    .attr('x1', 0).attr('y1', minorY).attr('x2', minorTickLineLength).attr('y2', minorY)
                                    .attr('stroke', defaultData.GridLineColorValue || 'black').attr('stroke-width', 1);
                            }
                        }
                    }
                }
