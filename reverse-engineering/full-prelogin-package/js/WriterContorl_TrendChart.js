...
export let WriterControl_TrendChart = {
    ...
    DataCalculationFun: function (rootElement) {
        ...
        if (targetCell != null) {
            ...
            //[DUWRITER5_0-4939] 计算宽度高度
            infos.width = parseFloat((targetCell.Width / 300 * 96.00001209449).toFixed(2));
            infos.height = parseFloat((targetCell.Height / 300 * 96.00001209449).toFixed(2)) - 8;
            console.log(infos);
            // //判断是否存在宽度
     ...
     * @param {number|HTMLElement|Object} [options.container] - 可选，指定容器（编辑器）或其 NativeHandle；也可使用 options.containerNativeHandle
     * @param {number} [options.containerNativeHandle] - 可选，指定容器（编辑器）元素 NativeHandle
     * @param {boolean} [options.clear] - 可选，是否在赋值前清空目标表格区域中“多余的表格行”数据（即本次数据行数之外的旧行）
     */
    TrendChartDataSourceToDocument: function (rootElement, data, options) {
        // console.log(data)
        // var startTime = new Date();
        var containerNativeHandle = null;
        var clearDocumentData = false;
        if (options && typeof options === "object") {
            // 容器：优先 containerNativeHandle，其次 container
            if (typeof options.containerNativeHandle === "number") {
                containerNativeHandle = options.containerNativeHandle;
            } else if (typeof options.container === "number") {
                containerNativeHandle = options.container;
            } else if (options.container && typeof options.container === "object") {
                // 允许传入包含 NativeHandle 的对象（如元素属性对象）
                if (typeof options.container.NativeHandle === "number") {
                    containerNativeHandle = options.container.NativeHandle;
                }
            }
            clearDocumentData = !!options.clear;
        }
        ...
        if (datasource) {
            ...
            if (typeof data == "object") {
                ...
                for (var i in data) {
                    ...
                    if (hasDataAttr) {
                        var tableNativeHandle = rootElement.GetElementByIdExt(hasDataAttr.tableID, containerNativeHandle);
                        var targetTable = rootElement.GetElementProperties(tableNativeHandle);
                        //开始循环赋值并合并单元格
                        var thisDataSourceData = data[i];
                        //获取到所有的表格行
                        var allRowHandles = targetTable.Rows;
                        var allRowEle = Array.isArray(allRowHandles) ? allRowHandles.slice() : allRowHandles;
                        ...
                        if (allRowEle.length - hasDataAttr.rowColStart[0] < thisDataSourceData.length) {
                            //对表格进行行新增
                            var insertRowOptions = {
                                FastMode: true, // yyf 2025-2-26 快速模式，不更新文档排版
                                tableid: tableNativeHandle,
                                rowindex: null,
                                rowcount: thisDataSourceData.length - (allRowEle.length - (hasDataAttr.rowColStart[0] - 1))
                            };
                            // var time223 = new Date();
                            rootElement.DCExecuteCommand("table_insertrowdown", false, insertRowOptions);
                            // console.log("插入表格行耗时：" + (new Date().valueOf() - time223.valueOf()));
                            //重新获取所有表格行
                            targetTable = rootElement.GetElementProperties(tableNativeHandle);
                            allRowHandles = targetTable.Rows;
                            allRowEle = Array.isArray(allRowHandles) ? allRowHandles.slice() : allRowHandles;
                        }
                            ...
                            rowItem.forEach((cellItem, cellIndex) => {
                                ...
                                if (Array.isArray(useItemText)) {
                                    ...
                                    if (isImageSource(itemDimensions.text)) {
                                        ...
                                        //判断文本是是否需要斜分线{ SlantSplitLineStyle:'TopRightOneLine'}
                                        let cellTextHasSplitLine = cellItem.text.indexOf('<SplitLine>');
                                        if (cellTextHasSplitLine >= 0) {
                                            rootElement.SetTableCellSlantTextExtByHandle(cellNativeHandle, useItemText, false, false);
                                        } else {
                        ...
                        // 赋值完成后再按需清空目标表格区域中“多余的表格行”旧数据（避免上次绑定残留）
                        if (clearDocumentData && thisDataSourceData.length > 0 && thisDataSourceData.length < allRowEle.length) {
                            var deleteRowCount = allRowEle.length - thisDataSourceData.length;
                            var deleteRowIndex = hasDataAttr.rowColStart[0] - 1 + thisDataSourceData.length;
                            if (deleteRowCount > 0) {
                                var deleteRowOptions = {
                                    FastMode: true, // 快速模式，不更新文档排版
                                    tableid: tableNativeHandle,
                                    rowindex: deleteRowIndex,
                                    rowcount: deleteRowCount
                                };
                                rootElement.DCExecuteCommand("table_deleterow", false, deleteRowOptions);
                            }
                        }
                        //rootElement.RefreshInnerView();
                        //刷新表格内数据
                        rootElement.EditorRefreshViewElement(tableNativeHandle);
                    }
                }
                // 清除缓存信息
    ...
    CreateTrendChartImage: function (rootElement) {
        ...
        if (CurrentTableCell) {
            ...
            WriterControl_TrendChart.GetSvgToPngBase64(svgNode, function (err, base64, svgWidth, svgHeight) {
                if (err) {
                    console.error('出错:', err);
                } else {
                    if (base64) {
                        // 使用SVG的宽高，需要转换为300DPI单位
                        // SVG的宽高是像素单位（96DPI），需要转换为300DPI单位
                        var svgWidthIn300DPI = (svgWidth / 96) * 300;
                        var svgHeightIn300DPI = (svgHeight / 96) * 300;
                        var id = "TrendChartCellImg" + new Date().getTime();
                        var options = {
                            Width: svgWidthIn300DPI,
                            Height: svgHeightIn300DPI,
                            id,
                            src: base64,
                            SaveContentInFile: true,
                            KeepWidthHeightRate: false,
                            SmoothZoom: false,
                            AutoFitImageSize: true      //根据SVG宽高自适应缩放
                        };
                        //插入图片到单元格
                        rootElement.DCExecuteCommand("InsertImage", false, options);
                        //判断单元格是否存在内边距，如果存在内边距则讲图片设置偏移量和宽高
                        if (targetCell && targetCell.Style) {
                            var targetCellPaddingTop = (targetCell.Style && targetCell.Style.PaddingTop) || 0;
                            var targetCellPaddingLeft = (targetCell.Style && targetCell.Style.PaddingLeft) || 0;

                            //如果存在内边距就使用图片偏移，铺满整个单元格
                            rootElement.SetElementProperties(id, {
                                ZOrderStyle: "UnderText",//为了不压线，置于文本下方
                                VAlign: "Top",
                                OffsetX: 0 - targetCellPaddingLeft + 2,//+2是为了让图片更贴近单元格边框
                                OffsetY: 0 - targetCellPaddingTop - 1,//-1是为了让图片更贴近单元格边框
    ...
    GetSvgToPngBase64: function (svg, callback) {
        ...
        // 获取SVG的实际宽高（优先使用native-width/height属性，如果没有则使用width/height属性，最后使用getBBox的结果）
        var svgWidth = parseFloat(svg.getAttribute('native-width')) || parseFloat(svg.getAttribute('width')) || b.width;
        var svgHeight = parseFloat(svg.getAttribute('native-height')) || parseFloat(svg.getAttribute('height')) || b.height;
        var wrap = document.createElementNS(NS, 'svg');
        wrap.setAttribute('xmlns', NS);
        wrap.setAttribute('viewBox', [0, 0, svgWidth, svgHeight].join(' '));
        wrap.setAttribute('width', svgWidth);
        wrap.setAttribute('height', svgHeight);
        ...
        img.onload = function () {
            ...
            callback(null, base64, svgWidth, svgHeight);
        };
