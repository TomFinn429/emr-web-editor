...
internal class Datatype_unsignedLong : Datatype_nonNegativeInteger
{
	...
	{
		return ((ulong)P_0).CompareTo((ulong)P_1);
	}
	...
	internal override Exception TryParseValue(string P_0, XmlNameTable P_1, IXmlNamespaceResolver P_2, out object P_3)
	{
		...
		if (ex == null)
		{
			ex = XmlConvert.TryToUInt64(P_0, out var num);
			if (ex == null)
			{
				ex = s_numeric10FacetsChecker.CheckValueFacets((decimal)num, (XmlSchemaDatatype)this);
				if (ex == null)
				{
					P_3 = num;
					return null;
